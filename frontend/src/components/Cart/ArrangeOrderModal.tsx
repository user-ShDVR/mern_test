...
export const ArrangeOrderModal = (props: IArrangeOrderModalProps) => {
  ...
  const onFinishAddOrder = async (values: any) => {
    console.log(values);
    ...
    const orderData: any = {
      ...
      address: `${values.locality}, ${values.street}, ${values.house}, ${values.flat}`,
    };
    ...
    try {
      ...
      if ('data' in response && response.data) {
        ...
        if ('status' in response.error) {
          message.error("Произошла ошибка при добавлении заказа: " + response.error.status);
